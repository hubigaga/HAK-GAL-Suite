...
class HAKGALAnalytics:
    ...
    def load_metrics(self):
        ...
        try:
            ...
            if self.metrics:
                ...
                print(f"✅ Loaded {len(self.metrics)} metrics from {self.metrics_file}")
            else:
    ...
    def generate_performance_insights(self) -> Dict[str, Any]:
        ...
        # Overall Performance
        successful_queries = self.df[self.df['success'] == True]
        insights['overall_performance'] = {
            ...
            'median_response_time': successful_queries['response_time_ms'].median(),
            'p95_response_time': successful_queries['response_time_ms'].quantile(0.95),
            'p99_response_time': successful_queries['response_time_ms'].quantile(0.99),
        ...
        if not error_data.empty:
            error_distribution = error_data['error_message'].value_counts().to_dict()
            timeout_rate = len(error_data[error_data['timeout_occurred'] == True]) / len(self.df) * 100
            ...
            insights['error_analysis'] = {
                ...
                'most_common_error': error_data['error_message'].mode().iloc[0] if not error_data.empty else None
            }
        ...
        insights['quality_metrics'] = {
            ...
            'total_learning_suggestions': self.df['learning_suggestions'].sum(),
    ...
    def _calculate_performance_trend(self) -> str:
        ...
        first_avg_time = first_quarter[first_quarter['success']]['response_time_ms'].mean()
        last_avg_time = last_quarter[last_quarter['success']]['response_time_ms'].mean()
        
        if last_success_rate > first_success_rate and last_avg_time < first_avg_time:
            return "improving"
        elif last_success_rate < first_success_rate or last_avg_time > first_avg_time:
            return "declining"
        else:
            return "stable"
    ...
    def _calculate_knowledge_growth_rate(self) -> float:
        ...
        initial_count = self.df['permanent_knowledge_count'].iloc[0] if not self.df.empty else 0
        final_count = self.df['permanent_knowledge_count'].iloc[-1] if not self.df.empty else 0
    ...
    def _calculate_learning_efficiency(self) -> float:
        ...
        return (total_facts / total_suggestions * 100) if total_suggestions > 0 else 0.0
